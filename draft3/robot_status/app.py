...
class Draft3:
    ...
    def init_web_server(self, gpio1, gpio2, ultrasonic, infrared, encoders, jointx1, jointx2, jointx3, us1, us2, us3, us4, left, center, right):
        ...
        def gpio1_finder():
            ...
            return render_template('index.html', GPIO1=self.gpio1, GPIO2=self.gpio2, ultrasonic=self.ultrasonic, IR=self.infrared, encoders=self.encoders, jointx1 = self.jointx1, jointx2=self.jointx2, jointx3 = self.jointx3, US1 = self.us1, US2 = self.us2, US3 = self.us3, US4 = self.us4, left=self.left, center=self.center, right=self.right)
        ...
        @app.route("/jointx1/", methods=['GET', 'POST'])
        def jointx1_finder():
            self.jointx1= random.randint(100, 150)
            return render_template('index.html', GPIO1=self.gpio1, GPIO2=self.gpio2, ultrasonic=self.ultrasonic, IR=self.infrared, encoders=self.encoders, jointx1 = self.jointx1, jointx2=self.jointx2, jointx3 = self.jointx3, US1 = self.us1, US2 = self.us2, US3 = self.us3, US4 = self.us4, left=self.left, center=self.center, right=self.right)

        
        @app.route("/jointx2/", methods=['GET', 'POST'])
        def jointx2_finder():
            self.jointx2= random.randint(100, 150)
            return render_template('index.html', GPIO1=self.gpio1, GPIO2=self.gpio2, ultrasonic=self.ultrasonic, IR=self.infrared, encoders=self.encoders, jointx1 = self.jointx1, jointx2=self.jointx2, jointx3 = self.jointx3, US1 = self.us1, US2 = self.us2, US3 = self.us3, US4 = self.us4, left=self.left, center=self.center, right=self.right)


        @app.route("/jointx3/", methods=['GET', 'POST'])
        def jointx3_finder():
